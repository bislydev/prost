...
impl<T> PathMap<T> {
    ...
    pub(crate) fn get<'a>(&'a self, fq_path: &'_ str) -> Iter<'a, T> {
        Iter::new(self, fq_path.to_string())
    }

    pub(crate) fn get_field<'a>(&'a self, fq_path: &'_ str, field: &'_ str) -> Iter<'a, T> {
        Iter::new(self, format!("{}.{}", fq_path, field))
    }

    #[allow(unused)]
    pub(crate) fn get_best<'a>(&'a self, fq_path: &'_ str) -> Option<&'a T> {
        self.find_best_matching(fq_path)
    }

    pub(crate) fn get_best_field<'a>(&'a self, fq_path: &'_ str, field: &'_ str) -> Option<&'a T> {
...
/// Given a fully-qualified path, retruns a sequence of paths:
/// - the path itself
...
mod tests {
    ...
    fn test_get_best() {
        ...
        // worst is global
        path_map.insert(".".to_owned(), 1);
        assert_eq!(Some(&1), path_map.get_best(".a.b.c.d"));
        assert_eq!(Some(&1), path_map.get_best("b.c.d"));
        assert_eq!(Some(&1), path_map.get_best_field(".a.b.c", "d"));

        // then prefix
        path_map.insert(".a.b".to_owned(), 2);
        assert_eq!(Some(&2), path_map.get_best(".a.b.c.d"));
        assert_eq!(Some(&2), path_map.get_best_field(".a.b.c", "d"));

        // then suffix
        path_map.insert("c.d".to_owned(), 3);
        assert_eq!(Some(&3), path_map.get_best(".a.b.c.d"));
        assert_eq!(Some(&3), path_map.get_best("b.c.d"));
        assert_eq!(Some(&3), path_map.get_best_field(".a.b.c", "d"));

        // best is full path
        path_map.insert(".a.b.c.d".to_owned(), 4);
        assert_eq!(Some(&4), path_map.get_best(".a.b.c.d"));
        assert_eq!(Some(&4), path_map.get_best_field(".a.b.c", "d"));
    }
